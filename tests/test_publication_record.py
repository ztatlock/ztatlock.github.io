...
class PublicationRecordTests(unittest.TestCase):
    def test_loads_published_thin_bundle_and_normalizes_semantics(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            record = load_publication_record(root, "2025-demo-paper")
            self.assertFalse(record.draft)
            self.assertFalse(record.local_page)
            self.assertEqual(publication_listing_group(record), "main")
            ...
            self.assertEqual(
                publication_full_venue_label(record),
                "Architectural Support for Programming Languages and Operating Systems",
            )
            self.assertEqual(publication_compact_venue_label(record), "ASPLOS")
    ...
    def test_loads_published_local_page_bundle(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            record = load_publication_record(root, "2024-demo-journal")
    ...
    def test_draft_can_omit_indexed_only_fields(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            record = load_publication_record(root, "2026-draft-demo")
    ...
    def test_rejects_duplicate_json_keys(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesRegex(PublicationRecordError, "duplicate JSON key"):
                load_publication_record(root, "2025-demo-paper")
    ...
    def test_rejects_pub_year_slug_mismatch(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesRegex(PublicationRecordError, "pub_year must match slug year 2025"):
                load_publication_record(root, "2025-demo-paper")
    ...
    def test_rejects_published_bundle_without_venue_short(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesRegex(PublicationRecordError, "missing venue_short"):
                load_publication_record(root, "2025-demo-paper")
    ...
    def test_rejects_unknown_pub_type(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesRegex(PublicationRecordError, "pub_type must be one of"):
                load_publication_record(root, "2025-demo-paper")
    ...
    def test_requires_primary_link_for_thin_bundle(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesRegex(PublicationRecordError, "missing primary_link"):
                load_publication_record(root, "2025-demo-paper")
    ...
    def test_rejects_unknown_identifier_key(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with self.assertRaisesRegex(PublicationRecordError, "unknown identifier keys"):
                load_publication_record(root, "2025-demo-paper")

    def test_publication_order_key_handles_year_only_records(self) -> None:
        no_date = PublicationRecord(
            slug="2025-year-only",
            draft=False,
            local_page=False,
            primary_link="publisher",
            title="Year Only",
            authors=(PublicationPerson(name="Author"),),
            time=PublicationTime(pub_year=2025),
            venue=PublicationVenue(full="DemoConf", short="DemoConf"),
        ...
        with_date = PublicationRecord(
            ...
            title="With Date",
            authors=(PublicationPerson(name="Author"),),
            time=PublicationTime(pub_year=2025, pub_date=date(2025, 1, 1)),
            venue=PublicationVenue(full="DemoConf", short="DemoConf"),
            classification=PublicationClassification(
                listing_group="main",
                pub_type="conference",
        ...
        self.assertLess(publication_order_key(with_date), publication_order_key(no_date))
