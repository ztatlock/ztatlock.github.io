...
- [ ] Plan and implement the CV route/wrapper cutover slice so the CV moves from `site/pages/cv.dj` to `site/cv/index.dj`, canonicalizes `/cv/`, and rewrites lingering `cv.html` links to `cv/` before any CV section projection begins (`docs/plans/cv-campaign.md`, `docs/plans/cv-slice-1-route-cutover.md`, `site/pages/cv.dj`, `site/pages/index.dj`, `site/pages/notes.dj`).
...
- [ ] Decide whether the current omission of Ian Briggs from the CV visiting-students section is intentional or drift to remove during the students CV slice (`site/students/index.dj`, `site/pages/cv.dj`, `docs/plans/cv-campaign.md`, `docs/plans/cv-slice-2-students-projection.md`).
