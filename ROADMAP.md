...
- [ ] Continue the teaching-staffing campaign with the tutor-decision slice so the distinct tutor role exposed by the canonical teaching-staffing research gets an explicit policy rather than being silently dropped or folded into `teaching_assistants` (`docs/plans/teaching-staffing-campaign.md`, `site/data/teaching.json`, `site/data/people.json`).
