...
- [ ] Continue the teaching-staffing campaign for courses where Zach was the instructor, starting with the people-linkability/ref-guardrail slice so `site/data/people.json` can represent linkable and linkless people honestly before later adding offering-level `co_instructors` and `teaching_assistants` to `site/data/teaching.json`; keep the current `teaching_assistant` history group separate and treat this as a likely unlock for the later collaborator relationship-model slice (`docs/plans/teaching-staffing-campaign.md`, `docs/plans/teaching-staffing-slice-1-people-linkability.md`, `docs/plans/teaching-campaign.md`, `docs/plans/collaborators-campaign.md`, `site/data/people.json`, `site/data/teaching.json`).
...
- [ ] Plan the collaborator relationship-model slice so non-coauthor collaborator facts can later live in a small collaborator-specific registry keyed by `site/data/people.json` without duplicating publication-based coauthorship; treat the teaching-staffing campaign as a likely prerequisite so teaching collaborators stay canonical in `site/data/teaching.json` instead of being copied prematurely into collaborator data (`docs/plans/collaborators-campaign.md`, `docs/plans/teaching-campaign.md`, `docs/plans/teaching-staffing-campaign.md`, `site/data/people.json`, `site/data/teaching.json`).
