...
- [ ] Add a later teaching-staffing slice for courses where Zach was the instructor, with offering-level `co_instructors` and `teaching_assistants` fields tied into `site/data/people.json`, without overloading the current `teaching_assistant` history group that records Zach's own prior TA experience; this is also a likely unlock for the later collaborator relationship-model slice because it would establish a second canonical source for non-coauthor teaching collaborators (`docs/plans/teaching-campaign.md`, `docs/plans/collaborators-campaign.md`, `site/data/teaching.json`, `site/data/people.json`).
...
- [ ] Plan the collaborator relationship-model slice so non-coauthor collaborator facts can later live in a small collaborator-specific registry keyed by `site/data/people.json` without duplicating publication-based coauthorship; treat the later teaching-staffing slice as a likely prerequisite so teaching collaborators stay canonical in `site/data/teaching.json` instead of being copied prematurely into collaborator data (`docs/plans/collaborators-campaign.md`, `docs/plans/teaching-campaign.md`, `site/data/people.json`, `site/data/teaching.json`).
