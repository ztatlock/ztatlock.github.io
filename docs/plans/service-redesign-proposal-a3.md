...
- [service-redesign-proposal-a2-patch-list-review-2.md](/Users/ztatlock/www/ztatlock.github.io/docs/plans/service-redesign-proposal-a2-patch-list-review-2.md)

## Purpose

This is the next refinement after A2.

The goal is to keep the same core semantic bet:

- canonical `series`
- canonical `run`
- atomic `instance`

while tightening the design in response to external review.

Compared to A2, A3 mainly strengthens:

- semantic clarity
- field ownership and inheritance rules
- validation rules
- promotion guidance
- the explicit contract between authored JSON and the loaded model

So A3 is not a new direction.
It is A2 with the most convincing review-driven clarifications folded in.
...
## Why A3 Still Prefers Canonical Runs

The core design judgment remains unchanged from A1 and A2:
...
The lighter authored forms are there to improve physical ergonomics, not to
retreat from canonical runs.
...
This is a real seam, but it is a manageable and explicit one.
...
- use `explicit series with runs` when:
  ...
  - or explicit run keys / run-level semantics need to be stated directly

This keeps the file compact and the diffs small while preserving the richer
loaded model.
...
- or explicit run keys beyond the series key are needed
...
- shorthand records reuse the series key as the implicit run key
...
- the original run should usually keep the old series key as its run key
...
## Why A3 Is Better Than A2

A3 keeps everything good about A2:

- compact physical JSON
- self-contained common cases
- canonical runs after loading

But it strengthens the parts A2 left too implicit:
...
So A3 is the same design, just more believable as a final spec.

## Remaining Open Questions

The remaining open questions are now narrow:

- exactly how strict validation should become around uncertainty in older
  historical service
